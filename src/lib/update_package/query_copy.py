...
Typical usage:
    ...
    new_df = get_df(proteins, attributes, functions, df)
"""
...
def get_df(
    ...
    if aggregate:
        ...
        df = df.groupby("entry", as_index=False).agg(aggregate_values)
    else:
